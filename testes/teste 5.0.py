...
def start_the_game(player1,player2):
    ...
    while run:
        ...
        if death_spedy_anime == 1:
            ...
            if death_spedy_status >= len(death_spedy):
                ...
                #spedy death
        if death_spedy_anime == 1:
            death_spedy_status += dt/60
            if death_spedy_status >= len(death_spedy):
                spedy_death_status = 0
                spedy_y = -650    
                spedy_x = 750
                death_spedy_anime = 0
            else:
        ...
        # 3. desenhar no ecra
        if spedy_vy > 0 or josefino_vy>0: #not is_in_plataform and spedy_vy > 0:  # saltar
            die_checker = [[josefino_x,josefino_y],[spedy_x,spedy_y]]
            for al in die_checker:
                for oi in die_checker:
                    if al == oi:
                        continue
                    elif al[0]+spedy_SIZE >= oi[0]  and al[0]<= oi[0] + spedy_SIZE:
                        if al[1]+spedy_size_actual_y/7 >= oi[1] and al[1] < oi[1]:
                            if spedy_vy >0 and death_josefino_anime == 0:
                                score[0]+=1
                                death_josefino_anime = 1
                            if josefino_vy > 0 and death_josefino_anime ==0:
                                score[1]+=1
                                death_spedy_anime = 1
        ...
        if score[1] >=4:
            screen.blit(winner,(250,100))
            screen.blit(pygame.font.SysFont(player2.get_value(),90).render(player2.get_value(),True ,color),(670, 275))
    ...
    pygame.quit()
    pass
menu = pygame_menu.Menu('spedy and Josefino', 1530, 880,theme=pygame_menu.themes.THEME_BLUE)
